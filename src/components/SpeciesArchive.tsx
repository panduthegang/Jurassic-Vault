        ...
        >
          {DINOSAURS.map((dino, idx) => (
            <div
              key={dino.id}
              data-carousel-card
              className="snap-start flex-shrink-0 w-[85vw] sm:w-[325px] md:w-[calc((100%-48px)/3)] rounded-3xl liquid-glass overflow-hidden group flex flex-col border border-white/[0.08] p-4 bg-white/[0.01]"
            >
              ...
              <div className="pt-5 flex flex-col justify-between flex-grow">
                ...
                {/* Liquid Glass Interactive Trigger Button */}
                <button
                  onClick={() => setSelectedSpecies(dino)}
                  className="w-full mt-auto py-2.5 rounded-full liquid-glass text-white/90 hover:text-white hover:bg-white/10 font-mono text-[9px] uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer border border-white/5 hover:border-white/10"
                >
